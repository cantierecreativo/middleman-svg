...
describe Middleman::Svg::TransformPipeline::Transformations::AriaAttributes do
  it "adds a role attribute to the SVG document" do
    document = Nokogiri::XML::Document.parse("<svg>Some document</svg>")
    transformation = Middleman::Svg::TransformPipeline::Transformations::AriaAttributes.create_with_value({})
  ...
  context "aria-labelledby attribute" do
    it "adds 'title' when a title element is present" do
      ...
      transformation = Middleman::Svg::TransformPipeline::Transformations::AriaAttributes.create_with_value(true)

      expect(Middleman::Svg::IdGenerator).to receive(:generate).with("title", "Some title").
        and_return("some-id")
    ...
    it "adds 'desc' when a description element is present" do
      ...
      expect(Middleman::Svg::IdGenerator).to receive(:generate).with("desc", "Some description").
        and_return("some-id")
    ...
    it "uses existing IDs when they exist" do
      ...
      expect(Middleman::Svg::IdGenerator).to receive(:generate).with("my-title", "Some title").
        and_return("some-id")
      expect(Middleman::Svg::IdGenerator).to receive(:generate).with("my-desc", "Some description").
        and_return("some-other-id")
